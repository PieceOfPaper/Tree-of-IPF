...
function WIKI_MAP_TROPHY_VIEW(frame)
	...
	while 1 do
		...
		if wikiIndex ~= -1 then
			if cls.Category == 'Map' then
				local mapCls = GetClass("Map", cls.TargetClassName);
				if mapCls ~= nil then
					local mapRootCategory = mapPicasa:AddCategory(mapCls.CategoryName, mapCls.CategoryName);
					if mapRootCategory ~= nil then
						local mapSubCategory  = mapRootCategory:AddCategory(mapCls.ClassName, mapCls.Name);

						local mapEventCls = GetClass("Map_Event_Reward", mapCls.ClassName);
						if mapEventCls ~= nil then
							local mapEventSubCategory  = mapSubCategory:AddCategory('MapEvent_'..mapCls.ClassName, ScpArgMsg('Auto_TeugByeolBoSang'), 1);

							local mapEventGroupBox = mapEventSubCategory:GetGroupBox();
							mapEventGroupBox:SetSkinName("mypage_bar");
							local groupBoxWidth = 400;
							MAP_EVENT_GBOX_ADD(groupBoxWidth, mapEventGroupBox, mapCls, mapEventCls);
						end
					end
				end
			elseif cls.Category == 'Monster' then
				local targetNameStart, targetNameEnd = string.find(cls.TargetClassName, '/');
				local monstercls = nil;

				if targetNameStart == nil then
					monstercls = GetClass("Monster", cls.TargetClassName);
				else
					local targetName = string.sub(cls.TargetClassName, 1, targetNameStart - 1);
					monstercls = GetClass("Monster", targetName);
				end
				if monstercls ~= nil then
					local targetMapName = cls.TargetMapName;

					while 1 do
						local mapNameStart, mapNameEnd = string.find(targetMapName, '/');
						if mapNameStart == nil then
							local mapCls = GetClass("Map", targetMapName);
							if mapCls ~= nil then
								local mapRootCategory = mapPicasa:AddCategory(mapCls.CategoryName, mapCls.CategoryName);
								local mapSubCategory  = mapRootCategory:AddCategory(mapCls.ClassName, mapCls.Name);
								local monsterSubCategory  = mapSubCategory:AddCategory('Monster', ScpArgMsg('Auto_MonSeuTeo'));
								local picasaItem      = monsterSubCategory:AddItem(monstercls.ClassName, cls.Illust, cls.Name, cls.Desc);
								picasaItem:SetValue(cls.ClassID);
							end
							break;
						end

						local mapName   = string.sub(targetMapName, 1, mapNameStart - 1);
						...
						targetMapName = string.sub(targetMapName, mapNameEnd + 1);
					end
				end
			elseif cls.Category == 'Npc' then
