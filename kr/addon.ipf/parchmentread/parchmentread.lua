...
function PARCHMENTREAD_ON_INIT(addon, frame)
	addon:RegisterMsg('READ_PARCHMENT', 'ON_READ_PARCHMENT')
end

function ON_READ_PARCHMENT(frame, msg, dialog, argNum)
    local maxPage = PARCHMENT_MAX_PAGE(dialog)
    ...
    PARCHMENT_VIEW_PAGE(frame, 1)
end

-- UI PART

function PARCHMENT_UI_INIT(frame)
    frame:SetUserValue("PAGE", 1)
    frame:SetUserValue("MAXPAGE", 0)
end

function PARCHMENT_UI_OPEN(frame)
    frame:RunUpdateScript("PARCHMENT_UPDATE", 0, 0, 0, 1)

    PARCHMENT_UI_INIT(frame)
end

function PARCHMENT_UI_CLOSE(frame)
	frame:ShowWindow(0)
...
function PARCHMENT_UPDATE(frame)
    if imcinput.HotKey.IsDown("MoveRight") == true then
		PARCHMENT_NEXT_PAGE(frame)
    end
    
    if imcinput.HotKey.IsDown("MoveLeft") == true then
		PARCHMENT_PREV_PAGE(frame)
	end

	return 1;
end

function PARCHMENT_MAX_PAGE(dialogName)
...
function PARCHMENT_PREV_PAGE(frame)
	...
	PARCHMENT_VIEW_PAGE(topFrame, page)
end

function PARCHMENT_NEXT_PAGE(frame)
    local topFrame = frame:GetTopParentFrame()
    local page = topFrame:GetUserValue("PAGE") + 1
