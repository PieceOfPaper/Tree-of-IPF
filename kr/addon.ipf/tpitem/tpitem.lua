...
function TPITEM_DRAW_ITEM(frame, category, subcategory)
	...
	for i = 0, cnt - 1 do
		...
		if obj.Category == category and obj.SubCategory == subcategory then
			...
			--y = (math.ceil( (index / 2) ) - 1) * (ui.GetControlSetAttribute("tpshop_item", 'height') * 1.1) -- 그룹박스 스크롤이 안나옴. 120으로 해도 안나옴
			...
			if itemobj.ItemType  ~= 'Equip' or result ~= "OK" or IS_ITEM_WILL_CHANGE_APC(itemobj.ClassID) ~= 1 then -- 그리고 펫이 아니라면
