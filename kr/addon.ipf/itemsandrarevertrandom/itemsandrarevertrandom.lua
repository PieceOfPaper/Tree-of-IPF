...
function ITEM_SANDRAREVERT_RANDOM_OPEN(frame)
	ui.OpenFrame("inventory")

	CLEAR_ITEM_SANDRAREVERT_RANDOM_UI()
...
function SENDOK_ITEM_SANDRAREVERT_RANDOM_UI()
	if ui.CheckHoldedUI() == true then
		return;
	end
...
function ITEM_SANDRAREVERT_RANDOM_DROP(frame, icon, argStr, argNum)
	...
	if FromFrame:GetName() == 'inventory' then
		...
		ITEM_SANDRAREVERT_RANDOM_REG_TARGETITEM(toFrame, iconInfo:GetIESID());
	end
end

-- 슬롯에 아이템 등록 시 아이템 옵션 관련 UI 정보 갱신
function ITEM_SANDRAREVERT_RANDOM_REG_TARGETITEM(frame, itemID)
	if ui.CheckHoldedUI() == true then
		return;
	end
...
function ITEM_SANDRAREVERT_RANDOM_EXEC(frame)
	...
	if materialCnt == '0' then
		ui.SysMsg(ClMsg("LackOfSandraRevertRandomMaterial"));
		return;
	end
	...
	ui.MsgBox_NonNested(clmsg, frame:GetName(), "_ITEM_SANDRAREVERT_RANDOM_EXEC", "_ITEM_SANDRAREVERT_RANDOM_CANCEL");
end

function _ITEM_SANDRAREVERT_RANDOM_CANCEL()
	local frame = ui.GetFrame("itemsandrarevertrandom");
end
...
function SUCCESS_SANDRAREVERT_RANDOM_OPTION(frame, msg, argStr, argNum)
	...
	ReserveScript("_SUCCESS_SANDRAREVERT_RANDOM_OPTION()", EFFECT_DURATION)
end

function _SUCCESS_SANDRAREVERT_RANDOM_OPTION()
	ui.SetHoldUI(false);
	local frame = ui.GetFrame("itemsandrarevertrandom");
	if frame:IsVisible() == 0 then
		return;
...
function UPDATE_REMAIN_SANDRA_GLASS_COUNT(frame)	
	local itemHaveCount = GET_INV_ITEM_COUNT_BY_PROPERTY({
        {Name = 'StringArg', Value ='Sandra_Glass'}
	}, false);
...
function REMOVE_SANDRAREVERT_RANDOM_TARGET_ITEM(frame)
	if ui.CheckHoldedUI() == true then
		return;
	end
...
function ITEM_SANDRAREVERT_RANDOM_INV_RBTN(itemObj, slot)
	local frame = ui.GetFrame("itemsandrarevertrandom");
	if frame == nil then
		return;
	end
