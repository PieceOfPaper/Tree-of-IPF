function ITEMSANDRA_6LINE_REVERT_RANDOM_ON_INIT(addon, frame)
	addon:RegisterMsg("SUCCESS_SANDRA_6LINE_REVERT_RANDOM_OPTION", "SUCCESS_SANDRA_6LINE_REVERT_RANDOM_OPTION");
end

-- 산드라의 정밀 감정돋보기 아이템을 이용해서 UI 오픈
function OPEN_SANDRA_6LINE_REVERT_RANDOM(invItem)
	for i = 1, #revertrandomitemlist do
		local frame = ui.GetFrame(revertrandomitemlist[i]);
		if frame ~= nil and frame:IsVisible() == 1 and revertrandomitemlist[i] ~= "itemsandra_6line_revert_random" then
			return;
		end
	end

	local frame = ui.GetFrame('itemsandra_6line_revert_random');
	frame:SetUserValue('REVERTITEM_GUID', invItem:GetIESID());
	frame:ShowWindow(1);
end

function ITEM_SANDRA_6LINE_REVERT_RANDOM_OPEN(frame)
	ui.OpenFrame("inventory");

	CLEAR_ITEM_SANDRA_6LINE_REVERT_RANDOM_UI();
	...
	INVENTORY_SET_CUSTOM_RBTNDOWN("ITEM_SANDRA_6LINE_REVERT_RANDOM_INV_RBTN");
end

function ITEM_SANDRA_6LINE_REVERT_RANDOM_CLOSE(frame)
...
-- UI 초기화
function CLEAR_ITEM_SANDRA_6LINE_REVERT_RANDOM_UI()
	if ui.CheckHoldedUI() == true then return; end
	...
	UPDATE_REMAIN_SANDRA_GLASS_6LINE_COUNT(frame);
end

-- 감정 후 확인 버튼 클릭시 UI 갱신
function SENDOK_ITEM_SANDRA_6LINE_REVERT_RANDOM_UI()
	if ui.CheckHoldedUI() == true then return; end
	...
	ITEM_SANDRA_6LINE_REVERT_RANDOM_REG_TARGETITEM(frame, iconInfo:GetIESID());
end

-- 슬롯에 아이템 드랍
function ITEM_SANDRA_6LINE_REVERT_RANDOM_DROP(frame, icon, argStr, argNum)
	if ui.CheckHoldedUI() == true then return; end
	...
	if FromFrame:GetName() == 'inventory' then
		...
		local ret, reason = IS_ENABLE_6LINE_REVERT_RANDOM_ITEM(itemObj)

		if ret == false then
			if reason == 'Level' then
				ui.SysMsg(ScpArgMsg('CanUseLV430'))
			elseif reason == 'Count' then
				ui.SysMsg(ClMsg("6lineRevertRandomItemEnableRandomOptionCount"));
		...
		ITEM_SANDRA_6LINE_REVERT_RANDOM_REG_TARGETITEM(toFrame, iconInfo:GetIESID());
	end
end

-- 슬롯에 아이템 등록 시 아이템 옵션 관련 UI 정보 갱신
function ITEM_SANDRA_6LINE_REVERT_RANDOM_REG_TARGETITEM(frame, itemID)
	if ui.CheckHoldedUI() == true then return; end
...
-- 재감정 버튼 클릭
function ITEM_SANDRA_6LINE_REVERT_RANDOM_EXEC(frame)
	...
	local curcount = GET_INV_ITEM_COUNT_BY_PROPERTY({
        {Name = 'StringArg', Value ='Sandra_Glass_MAX_6line'}
	}, false);

	if curcount == 0 then
		ui.SysMsg(ClMsg("LackOfSandra6LineRevertRandomMaterial"));
		return;
	end
	...
	local clmsg = ScpArgMsg("DoSandra6LineRandomResetOptionCountNoReset");
	ui.MsgBox_NonNested(clmsg, frame:GetName(), "_ITEM_SANDRA_6LINE_REVERT_RANDOM_EXEC", "_ITEM_SANDRA_6LINE_REVERT_RANDOM_CANCEL");
end

function _ITEM_SANDRA_6LINE_REVERT_RANDOM_CANCEL()
	
end

function _ITEM_SANDRA_6LINE_REVERT_RANDOM_EXEC()
	local frame = ui.GetFrame("itemsandra_6line_revert_random");
...
function SUCCESS_SANDRA_6LINE_REVERT_RANDOM_OPTION(frame, msg, argStr, argNum)
	...
	ReserveScript("_SUCCESS_SANDRA_6LINE_REVERT_RANDOM_OPTION()", EFFECT_DURATION);
end

function _SUCCESS_SANDRA_6LINE_REVERT_RANDOM_OPTION()
	ui.SetHoldUI(false);
...
-- 보유한 돋보기 갯 수 출력
function UPDATE_REMAIN_SANDRA_GLASS_6LINE_COUNT(frame)
...
function REMOVE_SANDRA_6LINE_REVERT_RANDOM_TARGET_ITEM(frame)
	if ui.CheckHoldedUI() == true then return; end
...
function ITEM_SANDRA_6LINE_REVERT_RANDOM_INV_RBTN(itemObj, slot)
	local frame = ui.GetFrame("itemsandra_6line_revert_random");
