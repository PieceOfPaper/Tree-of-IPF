                            ...
                            if IsSameObject(guildObj, OccupationGuild) == 1 then --자신의 길드가 점령 길드라면,
                                ...
                                if pet == nil or pet == "None" then --컴패니언이 동행되어 있지 않다면,
                                    ...
                                    if isActive == 0 then --컴패니언이 소환되어 있지 있다면,
                                        ...
                                        local list, cnt = SCR_GUILD_COLONY_RESTRICTION_CHECK(self, "GuildColony_unRestricted_Companion_Class")
                                        for i = 1, cnt do
                                            local jobCircle = GetJobGradeByName(pc, list[i])
                                            if jobCircle >= 1 then --해당 리스트 클래스를 1서클 이상 올렸다면,
                                                if list[i] == 'Char3_14' then --해당 클래스가 응사라면,
                                                    local Char3_2_Circle = GetJobGradeByName(pc, 'Char3_2')
                                                    local Char3_7_Circle = GetJobGradeByName(pc, 'Char3_7')
                                                    local Char3_10_Circle = GetJobGradeByName(pc, 'Char3_10')
                                                    if Char3_2_Circle >= 1 or Char3_7_Circle >= 1 or Char3_10_Circle >= 1 then --응사 클래스가 헌터 또는 하카펠 또는 슈바르츠라이터 클래스를 올렸다면,
                                                        return "COLONY", ZoneClsName, enterCls
                                                    end
                                                elseif list[i] == 'Char3_2' or list[i] == 'Char3_7' or list[i] == 'Char3_10' or list[i] == 'Char1_7' or list[i] == 'Char1_17' then --해당 클래스가 헌터 또는 슈바르츠라이터 또는 하카펠 또는 캐터프랙트 또는 랜서라면,
                                                    return "COLONY", ZoneClsName, enterCls
                                                end
                                            end
                                        end
    ...
    elseif state == 2 then --콜로니전이 시작 상태라면,
        ...
        if zone_cnt > 0 then --존 내 pc가 한명이상 있다면,
            ...
            if area_cnt > 0 then --타워 범위 안에 pc가 한명이상 있다면,
                ...
                for i = 1, area_cnt do
                    if IsDead(area_list[i]) == 0 then
                        ...
                        if IsSameObject(OccupationGuild, guildObj) == 0 then --점령 길드가 아니라면,
                            local list, cnt = SCR_GUILD_COLONY_RESTRICTION_CHECK(self, "GuildColony_Restricted_Buff_OccupationPoint")
                            local buff_check = nil
                            for j = 1, cnt do
                                if IsBuffApplied(area_list[i], list[j]) == 'YES' then
                                    buff_check = "YES"
                                    break
                                end
                            end
                            if buff_check ~= "YES" then
                ...
                SCR_GUILD_COLONY_OCCUPATION_POINT_UP_RUN(self, ZoneClsName, guildObjList_zone, guildObjList_area, range, num, maxPoint, onePoint, twoPoint, threePoint, fourPoint, removePoint)
...
function SCR_GUILD_COLONY_OCCUPATION_POINT_UP_RUN(self, ZoneClsName, guildObjList_zone, guildObjList_area, range, num, maxPoint, onePoint, twoPoint, threePoint, fourPoint, removePoint)
            ...
            if num_area < 4 then --타워 범위 내 길원이 4명 미만이라면,
        		if num_area == 1 then
        		    if checkPoint <= onePoint then
                        local guild_check = SCR_GUILD_COLONY_OCCUPATION_POINT_OTHER_GUILD_CHECK(self, ZoneClsName, guildObjList_type[back_i], guildObjList_type, guildObjList_area, maxPoint, onePoint, twoPoint, threePoint, fourPoint)
...
function SCR_GUILD_COLONY_OCCUPATION_POINT_OTHER_GUILD_CHECK(self, ZoneClsName, guildObj, guildObjList_type, guildObjList_area, maxPoint, onePoint, twoPoint, threePoint, fourPoint)
    for i = 1, #guildObjList_type do
...
function SCR_GUILD_COLONY_ENTER_START(self)
    ...
    AttachEffect(self, 'F_pattern017_green', 20.2, 'BOT', 0, height, 0, 1) --안전지대 경계선 이펙트 그린
end
