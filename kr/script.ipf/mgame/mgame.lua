...
function _SCR_INDUN_CONTRIBUTION_REWARD(pc, silver, exp, jobExp, itemName, itemCount, contribution, multipleRate, rank, indunClassID, pcIndunMultipleRate, mGameName)
    ...
    if nil == etcObj then
        ...
        if etcObj.IndunFreeTime == 'None' then
			...
			if isIndunMultipleValue == 1 and etcObj.IndunMultipleRate > 0 then 
                ...
                if pcInvList ~= nil and #pcInvList > 0 then
                    --ê¸°ê°„???„ì´??ì¹´ìš´???¼ìŠ¤?ì´ê¸??Œë¬¸???˜ë‚˜??êº¼ë‚´??ì²˜ë¦¬)
                    ...
                    --?„ì´??ê°?ˆ˜ ì²´í¬(?¸ë²¤? ë¦¬???ˆëŠ” ê²ƒê³¼, ?½ì´ ê±¸ë ¤?ˆì? ?Šì? ê¸°ê°„??ë°°ìˆ˜ê¶?
                    ...
                    if takeCnt <= multipleItemCount then
                        ...
                        -- ê¸°ê°„?œë? ?œì™¸???¨ì? ê°œìˆ˜ë§Œí¼ ì°¨ê°: ëª¨í—˜?¼ì? ?„ìš© ? í° ë¨¼ì?. ???•ë§ ?´ëŸ¬ê³??¶ì? ?Šë‹¤. ?´ê±° ?´ë–¡?˜ëƒ ì§„ì§œ
                        if takeCnt > 0 then
        ...
        if itemCount > 0 then
            -- ë³´ìŠ¤ ëª¬ìŠ¤?°ê? ê¸°ë³¸?¼ë¡œ ?œëž?˜ëŠ” ?ë¸Œ ê°œìˆ˜, 1
...
function MGAME_EVT_COND_PCCNT_IMMEDIATELY(cmd, curStage, eventInst, obj, limitCnt)
    local list, cnt = GetCmdPCList(cmd:GetThisPointer());
...
function MGAME_EVT_COND_PCCNT_OUT(cmd, curStage, eventInst, obj, limitCnt)
    local list, cnt = GetCmdPCList(cmd:GetThisPointer());
