...
function SCR_PVP_MINE_REWARD_RUN(pc)
    ...
    if ret == 'SUCCESS' then
        ...
        PVPMineResultLog(pc, teamName, result, myTeamPoint, 'misc_pvp_mine2', rewardItemCnt);
        PVPMinePointLog(pc, teamName, 'Delete', 'None', beforePoint, 0, 0, 'misc_pvp_mine2', rewardItemCnt)
        end
    end
end
    ...
    if aObj.PVP_MINE_MAX < 500 then
        ...
        if ret == "SUCCESS" then
            if IS_PC(pc) == true then
                PlayEffect(pc, "F_fireworks001", 1)
                PVPMinePointLog(pc, GetTeamName(pc), 'Get', 'BossKill', afterPoint - beforePoint, afterPoint);
            end
        end
    end
end
...
function SCR_PVP_MINE_DEAD(self)
    ...
    if IS_PC(killer) == true then
        ...
        PVPMineKillLog(killer, self, GetTeamName(killer));
    end
    PVPMineDeathLog(self, killer, GetTeamName(killer));
end
...
function SCR_EXCHANGE_PVP_MINE_DIALOG(self,pc)
    ...
    if ret == 'SUCCESS' then
        ...
        DO_MINEPVP_SCORE_UPDATE(pc)
        PVPMinePointLog(pc, GetTeamName(pc), 'Get', 'TradeCrystal', afterPoint - beforePoint, afterPoint, itemCount);
        return
    end
