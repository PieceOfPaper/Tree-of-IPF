...
function GET_PET_STAT(self, lv, statStr)
	...
	local stat = statRatio + statRatio / 25 * (lv * 1.5 + self.Level); -- 초기 컴패니언 능력치를 올려주기 위해 self.Level값을 더해줌
...
function PET_ATK(self)
    -- 컴패니언은 ATK 수치로 물리, 마법 공격력을 같이 계산
    local addLv = self.Level;
    local atk = self.Lv + self.STR + addLv + PET_ATK_BY_ABIL(self.Stat_ATK);
    -- 기본공격이 마법 공격인 펫이 등장하면 INT가 공격력에 더해지도록 예외처리가 필요
