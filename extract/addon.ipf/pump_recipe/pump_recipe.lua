...
function _PUMP_RECIPE_OPEN(frame, recipeType, itemName)
	...
	if recipeCls.NeedWiki == 1 and session.GetWikiByName(recipeCls.ClassName) == nil then
		return;
	end
	...
	for i = 1 , 5 do
		...
		countText:SetTextByKey("invItemCnt", tostring(invItemCnt));
		...
		if itemName == itemCls.ClassName then
