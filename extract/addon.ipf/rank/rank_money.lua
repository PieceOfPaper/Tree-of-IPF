...
function RANK_OPEN_MONEYTOP(ctrl, ctrlset, argStr, argNum)

	local frame = ui.GetFrame('rank')
	rank = GET_CHILD(frame, 'group_moneyTop', 'ui::CObject')
	rank:ShowWindow(1)

	RANK_SHOW_BACK_BUTTON(frame)

	imcSound.PlaySoundEvent('button_click_3');
	imc.client.statistics.api.lua.sync_rank_money('RANK_MONEYTOP_DRAW', '')
end

function RANK_PAGE_CHANGE_MONEYTOP(ctrl, ctrlset, argStr, argNum)
	local frame = ui.GetFrame('rank')
	local group = GET_CHILD(frame, 'group_moneyTop', 'ui::CGroupBox')
...
function RANK_MONEYTOP_DRAW(str)
	...
	local ranklist = imc.client.statistics.api.lua.get_ranklist('item_get_what_Vis_total')
	local myrank = imc.client.statistics.api.lua.get_my_rank('item_get_what_Vis_total')
	local count = imc.client.statistics.api.lua.get_data('item', 'get_what', 'Vis', 'total')
	...
	name:SetText(ScpArgMsg('Auto_{@st41}{#f0dcaa}PyeongKyun_SilBeo'))

	local score = GET_CHILD(element, 'score', 'ui::CRichText')
	score:SetText('{@st41}{#f0dcaa}'..imc.client.statistics.api.lua.get_money_top_avg())
