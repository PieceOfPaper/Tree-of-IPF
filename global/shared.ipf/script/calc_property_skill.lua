...
function SCR_Get_SpendSP(skill)
	...
	local abilAddSP = GetAbilityAddSpendValue(pc, skill.ClassName, "SP");
	-- lvUpSpendSP??Î£®ÏïÑ?êÏÑú??float ?ïÎ??ÑÎ? ?òÏ†ï?òÍ∏∞?ÑÌï¥ ?åÏàò 5?êÎ¶¨?êÏÑú Î∞òÏò¨Î¶ºÌïú??
	-- ?¥Î†áÍ≤?Í≥ÑÏÇ∞?¥Ï§ò???¥Îùº?¥Ïñ∏?∏Ï? Í≥ÑÏÇ∞ Í∞íÏù¥ ÎßûÎã§. ?ÑÎßà???åÏàò?êÏùò 10?êÎ¶¨ÏØ§Ïù¥ ?ÄÎ¶¨Ï? ?äÏùÑÍπ?.
...
function SCR_GET_USEOVERHEAT(skill)	
	...
	--skill.	
	local skillScale = 0.4; -- ?ÑÏãú -- skill.xml?êÏÑú ?ÖÎ†•	
...
function SCR_GET_SKILLLV_WITH_BM(skill)
	...
	if skill.GemLevel_BM > 0 then
		value = value + 1;	-- Î™¨Ïä§?∞Ï†¨ ?§ÌÇ¨Î≥¥ÎÑà?§Îäî Ï§ëÏ≤©?úÏºú??Î¨¥Ï°∞Í±?+1Îß??úÌÇ®?§Í≥†??
	end
