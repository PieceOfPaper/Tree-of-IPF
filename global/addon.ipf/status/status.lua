	...
	for i = 0, Selectclasslist:Count() do
		...
		if eachcls ~= nil then
			...
			if eachengname == nowengname then
				...
				-- ?ÖÏ†Å Î∞õÏúºÎ©??§Ïñ¥ Ïª¨Îü¨ ?¨ÎùºÏßÄ???ÑÏÉÅ???àÎã§Í≥??¥ÏÑú HairColor ?ÑÎ°ú?ºÌã∞ Í∞íÏúºÎ°úÎèÑ ?ïÏù∏
...
--Ï∫êÎ¶≠???¥Î¶Ñ Î≥ÄÍ≤?function CHANGE_MYPC_NAME_BY_ITEM(invItem)
	local newframe = ui.GetFrame("inputstring");
...
--?Ä ?¥Î¶Ñ Î≥ÄÍ≤?function CHANGE_TEAM_NAME_BY_ITEM(invItem)
	local newframe = ui.GetFrame("inputstring");
...
--Í∏∏Îìú ?¥Î¶Ñ Î≥ÄÍ≤?function CHANGE_GUILD_NAME_BY_ITEM(invItem)
	local newframe = ui.GetFrame("inputstring");
