...
function RANK_OPEN_LEVELTOP(ctrl, ctrlset, argStr, argNum)

	local frame = ui.GetFrame('rank')
	rank = GET_CHILD(frame, 'group_levelTop', 'ui::CObject')
	rank:ShowWindow(1)
	...
	imc.client.statistics.api.lua.sync_rank_level('RANK_LEVELTOP_DRAW', '')
end

function RANK_PAGE_CHANGE_LEVELTOP(ctrl, ctrlset, argStr, argNum)
	local frame = ui.GetFrame('rank')
	local group = GET_CHILD(frame, 'group_levelTop', 'ui::CGroupBox')
...
function RANK_LEVELTOP_DRAW(str)
	...
	local ranklist = imc.client.statistics.api.lua.get_ranklist('exp_level_up')
	local myrank = imc.client.statistics.api.lua.get_my_rank('exp_level_up')

	local count = GETMYPCLEVEL();
	...
	name:SetText(ScpArgMsg('Auto_{@st41}{#f0dcaa}PyeongKyun_LeBel'))

	local score = GET_CHILD(element, 'score', 'ui::CRichText')
	score:SetText('{@st41}{#f0dcaa}'..imc.client.statistics.api.lua.get_level_top_avg())
