...
function REINFORCE_MIX_UPDATE_ITEM_STATS(frame, obj, nextObj)
	...
	--???ÑÏö© ?àÎ≤®??prop?úÏãú ??™©. ?¨Ï? ??™© Î∂ÑÎ•òÍ∞Ä Î≤àÍ±∞Î°úÏö∞ÎØÄÎ°??ÑÏòà ?§Î•∏ ?úÍ≥º ?§Î•¥Í≤??åÎèÑÎ°??úÎã§. ?òÎ®∏ÏßÄ???ÅÌòë?®ÏóêÍ≤?Î¨∏Ïùò.
	if obj.GroupName == "Gem" then
		...
		for i = 1 , #propNameList do
			...
			if title ~= nil then
				-- Í∞??ÑÎ°ú?ºÌã∞ ÎßàÎã§ Ïª®Ìä∏Î°§ÏÖã ?ùÏÑ±. Î¨¥Í∏∞?êÏÑú ?úÎ≤à, ?ÅÏùò?êÏÑú ?úÎ≤à ?¥Îü∞ ??
				...
				local type_text = GET_CHILD(innerCSet,'type_text','ui::CRichText')
				-- Í∞??ÑÎ°ú?ºÌã∞???µÏÖò ÎßàÎã§ Ïª®Ìä∏Î°§ÏÖã ?ùÏÑ±. Í≥µÍ≤©??10?êÏÑú ?úÎ≤à, Î∏îÎü≠+10?êÏÑú ?úÎ≤à ?¥Îü∞ ??
...
function REINFORCE_MIX_INV_RBTN(itemObj, slot, selectall)
	...
	if 1 == _G[reinforceCls.MaterialScript](reinfItem, itemObj) then
		...
		if selectall == 'YES' then
			nowselectedcount = invitem.count -1 -- ?¥Ï∞®???§Ïóê??+1 ?òÎ?Î°?
		end
