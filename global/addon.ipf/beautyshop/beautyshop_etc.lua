-- beautyshop_etc.lua
local EtcItemList = nil

function ETCSHOP_OPEN()
    local topFrame = ui.GetFrame("beautyshop");
	if topFrame == nil then
		return
	end
	
	local shopName = topFrame:GetUserConfig("etcShopName");
	...
	ETCSHOP_GET_SHOP_ITEM_LIST(BEAUTYSHOP_GET_GENDER()) -- 아이템 목록 읽기 
end

function ETCSHOP_INIT_FUNCTIONMAP()
	-- Function Map 등록
	beautyShopInfo.functionMap["UPDATE_SUB_ITEMLIST"] = nil
	beautyShopInfo.functionMap["DRAW_ITEM_DETAIL"] = ETCSHOP_DRAW_ITEM_DETAIL
	beautyShopInfo.functionMap["POST_SELECT_ITEM"]= ETCSHOP_POST_SELECT_ITEM
...
function ETCSHOP_GET_SHOP_ITEM_LIST(gender)
	ETCSHOP_MAKE_ITEMLIST(gender)
end

function ETCSHOP_REGISTER_ITEM_LIST()
	-- Beauty_Shop_Lens.xml의 정보.
	
	if EtcItemList == nil then
		EtcItemList={}
		
		local clsList, cnt = GetClassList('Beauty_Shop_Lens');
		...
		for i = 0, cnt - 1 do
			...
			local data = {
                ...
                IDSpace = 'Beauty_Shop_Lens',
                ClassName = cls.ClassName,
			}

			table.insert(EtcItemList, data)
		end
	end
 ...
 function ETCSHOP_MAKE_ITEMLIST(gender)
    
	local list = ETCSHOP_REGISTER_ITEM_LIST()

	BEAUTYSHOP_UPDATE_ITEM_LIST(list, #list)
end

-- 헤어 아이템의 장착 타입을 가져옴.
function ETCSHOP_GET_ITEM_EQUIPTYPE(ItemClassName)
...
function ETCSHOP_POST_SELECT_ITEM(frame, ctrl)
	...
	local equipType = ETCSHOP_GET_ITEM_EQUIPTYPE(itemClassName)	-- itemClassName을 가지고 equipType을 가져옴.
...
function ETCSHOP_DRAW_ITEM_DETAIL(obj, itemobj, ctrlset)
