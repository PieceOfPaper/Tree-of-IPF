...
function ON_PARTY_UPDATE(frame, msg, str, num)
	...
	if pcparty == nil then
		...
		--?åÌã∞Î≥¥Ïä§ ?åÌôòÍ¥Ä?®Ìï¥???®ÏïÑ?àÏúºÎ©?ÏßÄ?åÎ≥¥??
...
function HIDE_PARTY_CREATE_BTN()
	local partyframe = ui.GetFrame('party') -- ?ºÎã® Î≤ÑÌäº???ÜÏóêÍ≥??òÏ§ë???ÖÎç∞?¥Ìä∏
...
function ON_PARTY_PROPERTY_UPDATE(frame, msg, str, num)
	...
	-- ?åÌã∞Îß§Ïπò ?µÏÖò ?§Ï†ï Í¥Ä??ÏΩîÎìú
...
function UPDATE_PARTY_TICKET(frame, msg, propName, propValue)
	...
	for i = 0 , cnt - 1 do
		...
		elseif cls.ViewProp ~= "None" then
			--??Î∞ëÏóê??Íµ¨Î≤Ñ???∏Ìôò???ÖÎãà?? Í∞?àòÍ∞Ä ÎßéÏïÑÏßÄÎ©??∞Î°ú Í¥ÄÎ¶¨ÌïòÍ≤????¥Î¥ê?ºÌï®.
...
function ACCEPT_PARTY_EVENT(parent, ctrl)
	...
	--desc:SetTextByKey("value", "ÎØ∏ÏÖò?úÏûë?");
...
function UPDATE_I_NEED_PARTY(frame, msg, str, num)
	...
	if mylevellimit == -1 then
		...
		-- 0001 : ?åÎìúÎß?/ 0010 : ?ÑÏûê??/ 0100 : ?ÑÏ≤ò / 1000 : ?¥Î†àÎ¶?
	...
	elseif pcparty ~= nil and isLeader == 0 then -- Î¶¨Îçî ?ÑÎãå ?ºÎ∞ò ?åÌã∞?êÏù¥?ºÎ©¥ Î¶¨Îçî???§Ï†ï ?∞ÎùºÍ∞?
		...
		while num > 0 do -- 2ÏßÑÏàò Î≥Ä?? ?§Îßà ÎπÑÌä∏?∞ÏÇ∞???àÎê† Ï§ÑÏù¥??,
...
function SET_I_NEED_PARTY()
	...
	if pcparty == nil then -- ?åÌã∞ ?çÌï¥?àÏ? ?äÎã§Î©?Í∑∏ÎÉ• ?ÅÏö©
	...
	if useineedparty == 0 then
		...
		if isLeader == 1 then -- Î¶¨Îçî?ºÎ©¥ ineedparty?Ä ?åÌã∞ ?ÑÎ°ú?ºÌã∞ ?ôÏãú ?ÅÏö©. ?ÑÎãàÎ©??†Ïùº ?ÜÎã§.
