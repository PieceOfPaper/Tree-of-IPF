...
function EXCHANGEANTIQUE_ON_INIT(addon, frame)
	...
	addon:RegisterMsg("DO_OPEN_EXCHANGE_ANTIQUE_UI", "EXCHANGEANTIQUE_MSG");
...
function EXCHANGEANTIQUE_MSG(frame, msg, argStr, argNum)
	...
	elseif msg == "DO_OPEN_EXCHANGE_ANTIQUE_UI" then
		frame:ShowWindow(1)
	end
end
...
function EXCHANGEANTIQUE_DROP(frame, icon, argStr, argNum)
	...
	if FromFrame:GetName() == 'inventory' then
		...
		ADD_ITEM_TO_EXCHANGEANTIQUE_FROM_INV(obj);
	end
end
...
function EXCHANGEANTIQUE_INV_RBTN(itemobj, slot)
	...
	CLEAR_EXCHANGEANTIQUE_UI()
	ADD_ITEM_TO_EXCHANGEANTIQUE_FROM_INV(itemobj)
...
function UPDATE_EXCHANGEANTIQUE_UI_BY_MSG(frame)
	...
	ADD_ITEM_TO_EXCHANGEANTIQUE_FROM_INV(item)

end


function ADD_ITEM_TO_EXCHANGEANTIQUE_FROM_INV(item)
...
function CLICK_EXCHANGE_BUTTON()
	...
	local str = ScpArgMsg("AntiqueExchange", "Silver", "SLOT", 1);
