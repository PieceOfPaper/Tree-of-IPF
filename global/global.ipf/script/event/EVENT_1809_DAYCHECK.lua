...
function SCR_EVENT_1809_DAYCHECK_TIME_CHECK()
    ...
    local EVENT_1809_DAYCHECK = {{{2018,10,30},{2018,11,13}}}
...
--            print('XXXXXXX',i, lastRewardList[table.find(lastRewardList, i)])
    ...
    if #rewardIndex == 0 then
        if aObj.EVENT_1809_DAYCHECK_ADDREWARD == 0 then
            local select = ShowSelDlg(pc, 0, 'EV161124_DAYCHECK_DLG7_STEAM', ScpArgMsg('Auto_JongLyo'), ScpArgMsg('EV161124_DAYCHECK_SEL1_STEAM'), ScpArgMsg('EV161124_DAYCHECK_SEL2_STEAM'))
            if select == 2 then
                local tx = TxBegin(pc);
                ...
                TxGiveItem(tx,'Hat_628335', 1, "EVENT_1809_DAYCHECK")    
            ...
            elseif select == 3 then
            	...
            	TxSetIESProp(tx, aObj, "EVENT_1809_DAYCHECK_ADDREWARD", 1);
                TxGiveItem(tx,'Premium_SkillReset_14d', 1, "EVENT_1809_DAYCHECK")
                TxGiveItem(tx,'Hat_628336', 1, "EVENT_1809_DAYCHECK")    
            	local ret = TxCommit(tx);
            end
        else
