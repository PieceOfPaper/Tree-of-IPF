...
-- ?àÎÇ¥Î©îÏÑ∏ÏßÄÎ°??ÑÏöî???ÑÏù¥?úÏùÑ Î≥¥Ïó¨Ï£ºÍ∏∞ ?ÑÌï®. 
...
-- ?¨Î£å???∞Î•∏ ?±Í≥µÎ•†Í≥º ?ÑÏù¥?úÏùò Ï¥àÏõî ?®Í≥Ñ ?ÖÎç∞?¥Ìä∏
...
-- ?±Í≥µÎ•†Ïóê ?∞Î•∏ Í∏Ä????Î≥Ä??
-- ?òÏ†ï ?ÑÏöî (?âÏÉÅÍ∞íÏù¥ ?ïÌï¥ÏßÄÏßÄ ?äÏïÑ???ÑÏßÅ ?åÎßû?Ä Í≥ÑÏÇ∞?ùÏùÑ Î™??∏Ïö∞Í≤†Ïùå. ?∞ÏÑ† ?òÎìúÏΩîÎî© ?¥ÎÜìÍ≤†Ïùå.)
...
function ITEM_TRANSCEND_REG_MATERIAL(frame, itemID)
	...
	--[[	
	-- Î©îÏÑ∏ÏßÄÎ∞ïÏä§Î°??òÎüâ?ºÎ°ú ?£Îäî Î∞©Î≤ï
...
function TRANSCEND_SET_MATERIAL_ITEM(frame, iesID, count)
	...
	local slot_material = GET_CHILD(frame, "slot_material");
	-- ?òÎüâ?úÏãúÎ•??¨Î°Ø???ÑÎ?Î∂ÑÏúºÎ°??òÏ†ï 
...
function ITEMTRANSCEND_EXEC(frame)
	-- ?πÏ†ï Î≤ÑÌîÑ ?¨Ïö© Ï§ëÏóê??Í∞ïÌôî/Ï¥àÏõî ÎßâÏïÑ?¨ÎùºÍ≥??òÏÖ®??
...
function ITEMTRANSCEND_INV_RBTN(itemObj, slot)
	...
	if slotInvItem ~= nil then
		if ("Premium_item_transcendence_Stone" == obj.ClassName) then
			ITEM_TRANSCEND_REG_MATERIAL(frame, iconInfo:GetIESID());	-- ?¨Î£å??Í≤ΩÏö∞
			return;
		end;
	end;
	ITEM_TRANSCEND_REG_TARGETITEM(frame, iconInfo:GetIESID());  -- ?¨Î£åÍ∞Ä ?ÑÎãê Í≤? Ï¥àÏõî ?πÌïò???ÑÏù¥??
end

-- ?†Îãà?ΩÏ≥ê???†ÎãàÎ©îÏù¥???±Ïóê ?∞Î•∏ Í≤∞Í≥º UIeffect ?§Ï†ï
...
-- ?úÎ≤Ñ???±Í≥µ?¨Î????∞Î•∏ UI?¥Ìéô?∏Ï? Í≤∞Í≥º ?ÖÎç∞?¥Ìä∏ 
...
-------------------------
-- Í≤∞Í≥º???∞Î•∏ UIeffectÍ∞Ä ?ùÏóÖ Í≤∞Í≥º UIÎ•?Í∞ÄÎ¶¨Îäî ?¥Ïú†Î°?
-- ?úÍ∞ÑÏ∞®Î°ú ?ùÏóÖ Í≤∞Í≥º UIÎ•??ÑÏõåÏ£ºÍ∏∞ ?ÑÌïú UpdateScript.
