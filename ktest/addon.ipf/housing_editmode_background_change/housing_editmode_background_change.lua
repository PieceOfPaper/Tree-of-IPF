...
function RESET_HOUSING_EDITMODE_CHANGE_BACKGROUND()
	local frame = ui.GetFrame("housing_editmode_background_change");
	local gbox_page = GET_CHILD_RECURSIVELY(frame, "gbox_page");
	
	local currentMapName = session.GetMapName();
	local currentMapClass = GetClass("Housing_Place", currentMapName);
	if currentMapClass == nil then
		return;
	end

	local type = TryGetProp(currentMapClass, "Type");
	if type == "Guild" then
		return;
	end
