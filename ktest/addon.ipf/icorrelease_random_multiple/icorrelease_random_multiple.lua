function ICORRELEASE_RANDOM_MULTIPLE_ON_INIT(addon, frame)
	addon:RegisterMsg("OPEN_DLG_ICORRELEASE_RANDOM_MULTIPLE", "ON_OPEN_DLG_ICORRELEASE_RANDOM_MULTIPLE")

	--성공시 UI 호출
	addon:RegisterMsg("MSG_SUCCESS_ICOR_RELEASE_RANDOM_MULTIPLE", "SUCCESS_ICOR_RELEASE_RANDOM_MULTIPLE")
...
function ON_ICORRELEASE_RANDOM_MULTIPLE_UPDATE_COLONY_TAX_RATE_SET(frame)
	...
	CLEAR_ICORRELEASE_RANDOM_MULTIPLE()
end

function ICORRELEASE_RANDOM_MULTIPLE_OPEN(frame)
	ui.CloseFrame('rareoption')
	CLEAR_ICORRELEASE_RANDOM_MULTIPLE()
	INVENTORY_SET_CUSTOM_RBTNDOWN("ICORRELEASE_RANDOM_MULTIPLE_INV_RBTN")
	...
	for i = 1, max_count do
		...
		slot:SetEventScript(ui.DROP, "ICORRELEASE_RANDOM_CTRL_ITEM_DROP")
		slot:SetEventScript(ui.RBUTTONUP, "REMOVE_ICORRELEASE_RANDOM_CTRL_TARGETITEM")
	end
end

function ICORRELEASE_RANDOM_MULTIPLE_CLOSE(frame)
...
function CLEAR_ICORRELEASE_RANDOM_CTRL(ctrlSet)
	local slot = GET_CHILD_RECURSIVELY(ctrlSet, "slot", "ui::CSlot")
	slot:ClearIcon()
...
function CLEAR_ICORRELEASE_RANDOM_MULTIPLE()
	if ui.CheckHoldedUI() == true then
		return
	end

	local frame = ui.GetFrame("icorrelease_random_multiple")
...
function ICORRELEASE_RANDOM_CTRL_ITEM_DROP(slot, icon, argStr, argNum)
	if ui.CheckHoldedUI() == true then
		return
	end
	...
	if FromFrame:GetName() == 'inventory' then
		...
		ICORRELEASE_RANDOM_CTRL_REG_TARGETITEM(ctrlSet, iconInfo:GetIESID())
		_UPDATE_RELEASE_RANDOM_MULTIPLE_COST(toFrame)
	end
end

-- 랜덤옵션 아이커 장착해제 
function ICORRELEASE_RANDOM_CTRL_REG_TARGETITEM(ctrlSet, itemID)
	...
	local inheritItemName = TryGetProp(invItemObj, 'InheritanceRandomItemName', 'None')
...
function ICORRELEASE_RANDOM_MULTIPLE_EXEC(frame)
	...
	WARNINGMSGBOX_FRAME_OPEN(clmsg, "_ICORRELEASE_RANDOM_MULTIPLE_EXEC", "_ICORRELEASE_RANDOM_MULTIPLE_CANCEL")
end

function _ICORRELEASE_RANDOM_MULTIPLE_CANCEL()
	local frame = ui.GetFrame("icorrelease_random_multiple")
end

function _ICORRELEASE_RANDOM_MULTIPLE_EXEC(checkRebuildFlag)
	local frame = ui.GetFrame("icorrelease_random_multiple")
	...
	if rebuild_flag == true then
		ui.MsgBox(ScpArgMsg('IfUDoCannotExchangeWeaponType'), '_ICORRELEASE_RANDOM_MULTIPLE_EXEC(false)', 'None')
		return
	end
    ...
    item.DialogTransaction("RELEASE_ITEM_ICOR_RANDOM_MULTIPLE", resultlist)
end

function SUCCESS_ICOR_RELEASE_RANDOM_MULTIPLE(frame)
	local frame = ui.GetFrame("icorrelease_random_multiple")

	local do_release = GET_CHILD_RECURSIVELY(frame, "do_release")
	do_release:ShowWindow(0)

	ReserveScript("_SUCCESS_ICOR_RELEASE_RANDOM_MULTIPLE()", 0.01)
end

function _SUCCESS_ICOR_RELEASE_RANDOM_MULTIPLE()
	local frame = ui.GetFrame("icorrelease_random_multiple")
	if frame:IsVisible() == 0 then
		return
	end
...
function REMOVE_ICORRELEASE_RANDOM_CTRL_TARGETITEM(slot)
	if ui.CheckHoldedUI() == true then
		return
	end
	...
	CLEAR_ICORRELEASE_RANDOM_CTRL(ctrlSet)
	_UPDATE_RELEASE_RANDOM_MULTIPLE_COST(frame)
end

function ICORRELEASE_RANDOM_MULTIPLE_INV_RBTN(itemObj, slot)
	local frame = ui.GetFrame("icorrelease_random_multiple")
	if frame == nil then
		return
	end
