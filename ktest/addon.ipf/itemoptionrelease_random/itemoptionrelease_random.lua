...
function ITEMOPTIONRELEASE_RANDOM_ON_INIT(addon, frame)
	addon:RegisterMsg("OPEN_DLG_ITEMOPTIONRELEASE_RANDOM", "ON_OPEN_DLG_ITEMOPTIONRELEASE_RANDOM");

	--성공시 UI 호출
	addon:RegisterMsg("MSG_SUCCESS_ITEM_OPTION_RELEASE_RANDOM", "SUCCESS_ITEM_OPTION_RELEASE_RANDOM");
...
function ON_OPTIONRELEASE_UPDATE_COLONY_TAX_RATE_SET(frame)
	...
	CLEAR_ITEMOPTIONRELEASE_RANDOM_UI();
end;

function ITEMOPTIONRELEASE_RANDOM_OPEN(frame)    
	ui.CloseFrame('rareoption');
	SET_OPTIONRELEASE_RANDOM_RESET(frame);
	CLEAR_ITEMOPTIONRELEASE_RANDOM_UI();
	INVENTORY_SET_CUSTOM_RBTNDOWN("ITEMOPTIONRELEASE_RANDOM_INV_RBTN");
	ui.OpenFrame("inventory");
end;

function ITEMOPTIONRELEASE_RANDOM_CLOSE(frame)
...
function CLEAR_ITEMOPTIONRELEASE_RANDOM_UI()
	if ui.CheckHoldedUI() == true then
		return;
	end;

	local frame = ui.GetFrame("itemoptionrelease_random");
...
function _UPDATE_RELEASE_RANDOM_COST(frame)
...
function ITEM_OPTIONRELEASE_RANDOM_DROP(frame, icon, argStr, argNum)
	...
	if FromFrame:GetName() == 'inventory' then
		...
		ITEM_OPTIONRELEASE_REG_TARGETITEM_RANDOM_OPTION(toFrame, iconInfo:GetIESID());
		_UPDATE_RELEASE_RANDOM_COST(toFrame);
	end;
end;

-- 랜덤옵션 아이커 장착해제 
function ITEM_OPTIONRELEASE_REG_TARGETITEM_RANDOM_OPTION(frame, itemID)
...
function SET_OPTIONRELEASE_RANDOM_RESET(frame)
--	reg:ShowWindow(0);
end;

function ITEMOPTIONRELEASE_RANDOM_EXEC(frame)
	...
	WARNINGMSGBOX_FRAME_OPEN(clmsg, "_ITEMOPTIONRELEASE_RANDOM_EXEC", "_ITEMOPTIONRELEASE_RANDOM_CANCEL");
end

function _ITEMOPTIONRELEASE_RANDOM_CANCEL()
	local frame = ui.GetFrame("itemoptionrelease_random");
end;

function _ITEMOPTIONRELEASE_RANDOM_EXEC(checkRebuildFlag)
	local frame = ui.GetFrame("itemoptionrelease_random");
	...
	if checkRebuildFlag ~= false then
		if TryGetProp(invItemObj, 'Rebuildchangeitem', 0) > 0 then
			ui.MsgBox(ScpArgMsg('IfUDoCannotExchangeWeaponType'), '_ITEMOPTIONRELEASE_RANDOM_EXEC(false)', 'None');
			return;
		end;
	end;
	...
	pc.ReqExecuteTx_Item("RELEASE_ITEM_ICOR_RANDOM", invItem:GetIESID());
	return;
end;

function SUCCESS_ITEM_OPTION_RELEASE_RANDOM(frame)
	local frame = ui.GetFrame("itemoptionrelease_random");
...
function _SUCCESS_ITEM_OPTION_RELEASE_RANDOM()	
	...
	RELEASE_SUCCESS_EFFECT_RANDOM(frame);

	return;
end;

function RELEASE_SUCCESS_EFFECT_RANDOM(frame)
	local frame = ui.GetFrame("itemoptionrelease_random");
	local RELEASE_SUCCESS_EFFECT_NAME = frame:GetUserConfig('RELEASE_SUCCESS_EFFECT_RANDOM');
...
function REMOVE_OPTIONRELEASE_RANDOM_TARGET_ITEM(frame)
	if ui.CheckHoldedUI() == true then
		return;
	end;
...
function ITEMOPTIONRELEASE_RANDOM_INV_RBTN(itemObj, slot)
	local frame = ui.GetFrame("itemoptionrelease_random");
	if frame == nil then
		return;
	end;
	...
	if invItem ~= nil then
		CLEAR_ITEMOPTIONRELEASE_RANDOM_UI();
		ITEM_OPTIONRELEASE_REG_TARGETITEM_RANDOM_OPTION(frame, iconInfo:GetIESID());
		_UPDATE_RELEASE_RANDOM_COST(frame)
	end;
end;
