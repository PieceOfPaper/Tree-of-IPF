...
function BARRACK_GET_CHAR_INDUN_ENTRANCE_COUNT(cid, resetGroupID)
	...
	if indunCls.WeeklyEnterableCount ~= nil and indunCls.WeeklyEnterableCount ~= "None" and indunCls.WeeklyEnterableCount ~= 0 then
		if indunCls.UnitPerReset == 'PC' then
			return accountInfo:GetBarrackCharEtcProp(cid,'IndunWeeklyEnteredCount_'..resetGroupID)  --Îß§Ï£º ?®Ï? ?üÏàò
		else
			return(acc_obj['IndunWeeklyEnteredCount_'..resetGroupID])   							--Îß§Ï£º ?®Ï? ?üÏàò
		end
        
	else
		if indunCls.UnitPerReset == 'PC' then
			return accountInfo:GetBarrackCharEtcProp(cid, 'InDunCountType_'..resetGroupID);         --Îß§Ïùº ?®Ï? ?üÏàò
		else
			return (acc_obj['InDunCountType_'..resetGroupID]);            							--Îß§Ïùº ?®Ï? ?üÏàò
		end        
    end
end
...
function BARRACK_GET_INDUN_MAX_ENTERANCE_COUNT(resetGroupID)
    ...
    local bonusCount = 0;
    if indunCls.WeeklyEnterableCount ~= nil and indunCls.WeeklyEnterableCount ~= "None" and indunCls.WeeklyEnterableCount ~= 0 then
        return indunCls.WeeklyEnterableCount + bonusCount;  --Îß§Ï£º max
    else
        return indunCls.PlayPerReset + bonusCount;          --Îß§Ïùº max
    end
end
...
function UPDATE_BARRACK_MODE(frame)
	...
	elseif argStr == "Visit" then
		-- ?§Î•∏ ?ôÏÜå Î∞©Î¨∏?†Îïê Ï∫êÎ¶≠?ùÏÑ±Í¥Ä??Î≤ÑÌäº?Ä ?®Í∏¥??
		SHOW_BTNS(frame, 0)
...
function UPDATE_BARRACK_PET_BTN_LIST()
	...
	--?§Î•∏ ?†Ï????ôÏÜåÎ•?Î∞©Î¨∏????Í∑??†Ï???Ïª¥Ìéò?àÏñ∏??Ï∞æÍ∏∞ ?ÑÌï¥ Î∞©Î¨∏Ï§ëÏûÑ???åÎ†§Ï§Ä?
	local charlist = ui.GetFrame("barrack_charlist");
