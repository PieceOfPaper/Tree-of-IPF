...
function IS_REBUILD_CARE_ADD_PR_TARGET(invItem, exchangeItem)
	...
	if invItem.ItemGrade == 5 and invItem.UseLv == 360 and invItem.ClassType == 'Cannon' and invItem.DBLHand == 'NO' then -- (구) 벨코퍼 캐논
		if exchangeItem.ItemGrade == 5 and exchangeItem.UseLv == 360 and exchangeItem.DBLHand == 'YES' and exchangeItem.ClassType == 'Cannon' then -- 벨코퍼 캐논으로 바꿀 때
			return true;
		end
	end
	return false;
