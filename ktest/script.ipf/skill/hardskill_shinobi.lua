...
function DUMMYPC_REMOVE_BM(self, dpc)
    if self.PATK_BM > 0 then
        dpc.MINPATK = self.MINPATK - self.PATK_BM
        dpc.MAXPATK = self.MAXPATK - self.PATK_BM
    end
    
    if dpc.PATK_RATE_BM > 0 then
        dpc.MAXPATK = self.MAXPATK / (1 + self.PATK_RATE_BM)
        dpc.MINPATK = self.MINPATK / (1 + self.PATK_RATE_BM)
    end
end