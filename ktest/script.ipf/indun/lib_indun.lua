...
--[[ 레이드 입장횟수 기능 아직 하는 중
function RESET_INDUN_COUNT_OF_WEEK(pc, tx) --HS_COMMENT: 하는중이닷
    local clslist, cnt = GetClassList("Indun");
    if nil == clslist then
        return;
    end
    ...
    -- IsRequiredIndunResetOfWeek(time, dayOfWeek)
    --      현재 시간이 리셋시간 지났는지
    --      이미 초기화 시간이 세팅되었는지
    ...
    for i = 0, cnt - 1 do
        local pCls = GetClassByIndexFromList(clslist, i);
        local countType = "IndunCountOfWeek_" .. pCls.PlayPerResetType;
            TxSetIESProp(tx, etcObj, countType, 0);
        end

    local nextResetTimeOfWeek = 0; --hs_comment: GetNextIndunResetTimeOfWeek();
    TxSetIESProp(tx, etcObj, "Indun_ResetTimeOfWeek", nextResetTimeOfWeek);
    ...
    if ret == "SUCCESS" then
        SendProperty(pc, etcObj);
        -- SendAddOnMsg(pc, "INDUN_COUNT_RESET"); -- 쓰지도 않는데 왜 메세지 뿌리지.
    end
end
]]--
...
function MGAME_CHECK_INDUN(pc, cmd, indunName)
    ...
    local maxCount = indunCls.PlayerCnt;
    ...
    local indunCount = etcObj["InDunCountType_" .. indunCls.PlayPerResetType];
    local countType = "InDunCountType_" .. indunCls.PlayPerResetType;
    ...
    if etcObj.IndunFreeTime == 'None' then
        ...
        if didit ~= 1 and indunCls.AdmissionItemName == 'None' then   
            TxAddIESProp(tx, etcObj, countType, count);
        end
...
function IS_HAVE_INDUN_TICKET(pc, indunName)
    local etcObj = GetETCObject(pc);
    if etcObj == nil then
        return 0;
    end
    ...
    if admissionItemName == "None" or admissionItemName == nil or admissionItemCount == 0 then    
        ...
        if indunCount >= maxPlayCnt then
            return 0;
        end
    else
        local invAdmissionItemCount = GetInvItemCount(pc,admissionItemName);
        local admissionTakeItemCount = GET_ADMISSION_ITEM_TAKE_COUNT(pc, pCls);
...
function REQ_MOVE_TO_INDUN(pc, indunName, joinMethod, randomMission)
    ...
    if indunCls.Level > pc.Lv + empoweringLv then
        local scp = string.format("INDUN_CANNOT_YET(\'%s\')", "YourLevelIsLowThenIndun");
        ExecClientScp(pc, scp);
        return;
    end

    local haveIndunTicket = IS_HAVE_INDUN_TICKET(pc, indunCls.ClassName);

    local indunType = indunCls.ClassID;

    local playPerResetType = TryGetProp(indunCls, 'PlayPerResetType');
    ...
    if joinMethod == 2 then -- 자동 매칭 신청한 경우
        ...
        if admissionItemName == "None" or admissionItemName == nil or admissionItemCount == 0 then
            ...
            if haveIndunTicket == 0 then
                local scp = string.format("INDUN_CANNOT_YET(\'%s\')", "CannotJoinIndunItemScarcity");
                ExecClientScp(pc, scp);
                return;
            end
        end
