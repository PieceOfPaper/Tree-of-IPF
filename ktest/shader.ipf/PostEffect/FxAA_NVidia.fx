...
As of the FXAA 3.9 release I have not tested this code.
If it does not work, please let me know so I can fix it.
------------------------------------------------------------------------------
Extra requirements,
(1.) Different inputs: no posPos.
(2.) Different inputs: alias three samplers with different bias settings!
(3.) New constants: setup c4 through c6 as described below.
============================================================================*/
#if (FXAA_360_OPT == 1)
/*--------------------------------------------------------------------------*/
// width = screen width in pixels
// height = screen height in pixels
DECLARE_PARAMETER(float4, fxaaConstDir, c4);   // float4(1.0, -1.0, 0.25, -0.25);
DECLARE_PARAMETER(float4, fxaaConstInner, c5); // float4(0.5/width, 0.5/height, -0.5/width, -0.5/height);
DECLARE_PARAMETER(float4, fxaaConstOuter, c6); // float4(8.0/width, 8.0/height, -4.0/width, -4.0/height);
/*--------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------*/
    float4 lumaNwNeSwSe;
    asm {
        tfetch2D lumaNwNeSwSe.w___, tex[0], pos.xy, OffsetX = -0.5, OffsetY = -0.5, UseComputedLOD=false
        tfetch2D lumaNwNeSwSe._w__, tex[0], pos.xy, OffsetX =  0.5, OffsetY = -0.5, UseComputedLOD=false
        tfetch2D lumaNwNeSwSe.__w_, tex[0], pos.xy, OffsetX = -0.5, OffsetY =  0.5, UseComputedLOD=false
        tfetch2D lumaNwNeSwSe.___w, tex[0], pos.xy, OffsetX =  0.5, OffsetY =  0.5, UseComputedLOD=false
    };    
/*--------------------------------------------------------------------------*/
    lumaNwNeSwSe.y += 1.0/384.0;
...
/*--------------------------------------------------------------------------*/
    float4 rgbyM = tex2Dlod(tex[0], float4(pos.xy, 0.0, 0.0));
...
/*--------------------------------------------------------------------------*/
    float4 rgbyN1 = tex2Dlod(tex[1], float4(pos.xy + dir1.xy, 0.0, 0.0));
    float4 rgbyP1 = tex2Dlod(tex[1], float4(pos.xy + dir1.zw, 0.0, 0.0));
    float4 rgbyN2 = tex2Dlod(tex[2], float4(pos.xy + dir2.xy, 0.0, 0.0));
    float4 rgbyP2 = tex2Dlod(tex[2], float4(pos.xy + dir2.zw, 0.0, 0.0));
/*--------------------------------------------------------------------------*/
